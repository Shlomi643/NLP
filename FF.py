...
class FirstFormatter:
    ...
    @staticmethod
    def is_person(line):
        return line.startswith(FirstFormatter.person_spc)

    @staticmethod
    def is_text(line):
        return line.startswith(FirstFormatter.text_spc)

    @staticmethod
    def get_person(line):
        matcher = re.match(FirstFormatter.name_format, line)
        return None if matcher is None or line.startswith('(') else matcher.group(0)[:-2]

    @staticmethod
    def get_text(line):
        return re.split(FirstFormatter.name_format, line).pop().lstrip().lower()

    @staticmethod
    def get_script_list(lst):
        ret = []
        for line in lst:
            ret.append({'person': FirstFormatter.get_person(line).rstrip(' '), 'text': FirstFormatter.get_text(line)})
        return ret
    ...
    def get_script_map(soup):
        ...
        for td in text:
            line = td.string
            if FirstFormatter.is_person(line):
                line = (line[3:line.index('(')] if '(' in line else line[3:]) + ':: '
                if FirstFormatter.get_person(line) is None:
                    print(line)
                    continue
            elif FirstFormatter.is_text(line):
                curr = ret.pop()
                line = curr + line[3:] + ' '
            else:
                continue
            line = re.sub("\\(.*\\)", "", line)
            ret.append(line)
        return FirstFormatter.get_script_list(ret)
