...
class ThirdFormatter:
    ...
    def __init__(self, name):
        ...
        self.script_map = ThirdFormatter.get_script_map(self.soup)

    @staticmethod
    def get_person(line):
        return re.match(ThirdFormatter.name_format, line).group(0)[:-1]

    @staticmethod
    def get_text(line):
        return re.split(ThirdFormatter.name_format, line).pop().lstrip().lower()

    @staticmethod
    def get_script_list(lst):
        ret = []
        for line in lst:
            ret.append({'person': ThirdFormatter.get_person(line).rstrip(' '), 'text': ThirdFormatter.get_text(line)})
        return ret
    ...
    def get_script_map(soup):
        ...
        return ThirdFormatter.get_script_list(ret)
